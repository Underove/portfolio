...
%-------------------------------------------------------------------------------
\cvsection{1. ToBig's 20th Conference : Pi-FiRi — 엣지 디바이스 실시간 객체 탐지(화재·연기) 로봇}

\projsummary{라즈베리파이 로봇 위에서 경량 객체 탐지(EfficientDet-Lite) 모델을 실시간 구동하고, 추론 분리와 LLM Agent 보고까지 잇는 엣지 Vision AI 시스템 --- 투빅스 4인 팀 (데이터셋 구성·파인튜닝 팀 공동, 분산 추론 구조·모델 선정·LLM 연동 본인 주도)}
\projfigL[5.6cm]{p5.jpg}
...
\projfigL[6.4cm]{p5pipe.png}

\projpart{\faGear\hspace{1.3mm}시스템 및 하드웨어 구성}
\begin{cvitems}
  \item {\textbf{모델 선정 (EfficientDet-Lite1)}\enskip 엣지 지연 예산을 먼저 고정하고 TFLite 배포가 가능한 Lite0--Lite2를 지연--정확도로 비교, 공식 벤치마크(Pixel 4·CPU 4스레드/COCO 2017) 지연 49ms·mAP 30.55\%인 Lite1을 채택. 공개 화재·연기 데이터셋(Roboflow Universe)을 확보·증강해 파인튜닝(팀 공동)하고, 초기 모델(Lite0)에서 Lite1로 교체하며 탐지 성능을 재검증.}
...
\begin{substeps}
  \item 엣지 환경(라즈베리파이)에서 주행 제어와 고부하 AI 추론을 동시에 수행하면 연산 지연으로 \textbf{1 FPS 미만}까지 떨어져, 실시간 구동 자체가 불가능.
  ...
  \item 다만 이 구조는 \textbf{로컬 PC 의존과 무선 구간 지연}이라는 비용을 지며, 서버가 멈추면 탐지가 정지하고 주행만 유지 --- 당시 하드웨어 제약에서의 차선책으로, 온디바이스로 되돌리려면 양자화·프루닝 등 모델 경량화로 지연 예산 안에 넣는 것이 다음 과제.
\end{substeps}
...
\begin{substeps}
  ...
  \begin{substeps2}
    ...
    \item \textbf{순찰 보고서 생성} --- 순찰 시작·종료 시간, 구간별 통과 로그, 화재 이벤트를 사실에 근거해 작성. ``제공된 데이터에 없는 내용을 생성하지 말 것''을 포함한 다중 제약 프롬프트로 보고 범위를 통제하고, \texttt{temperature=0}으로 같은 입력에 같은 결과를 내는 재현성과 형식 일관성을 확보.
