...
\begin{cvitems}
  \item {\textbf{모델 선정 (EfficientDet-Lite1)}\enskip 엣지 지연 예산을 먼저 고정하고 TFLite 배포가 가능한 Lite0--Lite2를 지연--정확도로 비교, 공식 벤치마크 지연 49ms·mAP 30.55\%인 Lite1을 채택. 공개 화재·연기 데이터셋(Roboflow Universe)을 확보·증강해 파인튜닝(팀 공동)하고, 초기 모델(Lite0)에서 Lite1로 교체하며 탐지 성능을 재검증.}
