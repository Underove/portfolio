...
%-------------------------------------------------------------------------------
\cvsection{1. ToBig's 20th Conference : Pi-FiRi — 엣지 디바이스 기반 실시간 화재 감지 로봇}

\projsummary{라즈베리파이 로봇 위에서 경량 객체탐지 모델을 실시간으로 굴리고, 분산 추론과 LLM Agent로 자율 순찰·화재 대응까지 잇는 엣지 AI 시스템 --- 4인 팀, 아키텍처 설계·LLM 연동 담당}
...
\stephead{1}{분산 추론 아키텍처 설계 (엣지 연산 한계 극복 · 1 $\rightarrow$ 약 10 FPS)}
\begin{substeps}
...
\stephead{2}{경량 탐지 모델 선정 및 오탐 억제 규칙}
\begin{substeps}
  \item 엣지 실시간 요구(프레임당 수십 ms)를 먼저 고정하고, TFLite로 배포 가능한 EfficientDet-Lite 계열(Lite0--Lite2)을 지연--정확도 트레이드오프로 비교 --- 공식 벤치마크 기준 지연 49ms · mAP 30.55\%인 \textbf{Lite1}을 채택 (더 정확한 상위 모델은 지연 초과로 탈락).
  \item Roboflow Universe의 Fire\&Smoke Dataset을 수집·증강해 파인튜닝하고, 초기 모델(model0)을 교체하며 정확도--속도 균형을 재확인.
  \item 단일 프레임 노이즈로 인한 오탐을 막기 위해 \textbf{신뢰도 0.5 이상 탐지가 1.2초 연속 유지될 때만} 화재로 확정 --- 단발 탐지로 판정하지 않는 규칙을 코드로 강제.
\end{substeps}

\stephead{3}{LLM Agent 상황 보고 (Solar Pro2)}
\begin{substeps}
  ...
  \begin{substeps2}
    ...
    \item \textbf{순찰 보고서 생성} --- 순찰 시작·종료 시간, 구간별 통과 로그, 화재 이벤트를 사실에 근거해 작성. \texttt{temperature=0}과 다중 제약 시스템 프롬프트로 같은 입력에 같은 결과를 내는 재현성과 형식 일관성을 확보 --- 화재 확정은 임계값 규칙이 내리고 LLM은 보고문 변환만 담당.
...
\stephead{5}{PID 제어 로직을 통한 주행 안정화 (하드웨어 한계 보완 · 팀 공동)}
\begin{substeps}
