...
%-------------------------------------------------------------------------------
% ── 성과 한 줄이 다음 페이지로 밀려서, 이 프로젝트만 세로 여백을 조금 조임 (사진 크기는 유지)
...
\begin{cvitems}
  ...
  \item {\textbf{기술 스택}\enskip \pill{Python} \pill{PyTorch} \pill{Pandas} \pill{NumPy} \pill{Scikit-learn} \pill{1D CNN} \pill{BiLSTM}}
\end{cvitems}
...
\begin{substeps}
  ...
  \item \textbf{외부 메타 선별} --- 객실·단체·기상·시설 이용객을 전 항목 병합한 뒤 Fold 검증으로 기여도를 측정, 화담숲 이용객 수·스키장 1일 내장객 수만 최종 채택.
\end{substeps}
...
\begin{substeps}
  \item MSE는 판매량이 큰 메뉴 오차에 편향돼 소량 메뉴 예측력이 저하 --- MSE로 기본 패턴을 Warm-up한 뒤, 평가지표 SMAPE를 원 판매량 단위로 역변환해 손실로 직접 사용(가중 영업장 3배 가중 반영) --- \textbf{SMAPE 0.675 → 0.527, 약 22\% 개선}.
  \item 본선 평가 산식 (SMAPE+NMAE+NRMSE+Pearson $R^2$)/4를 미분 가능한 복합 손실로 재구현 --- 스케일 없는 NMAE·NRMSE에 상한 1을 걸어 저판매 메뉴의 손실 지배를 차단, $R^2$는 $(1-R^2)$로 방향 통일 --- \textbf{약 6\% 추가 향상}.
\end{substeps}

\stephead{5}{24-Fold 롤링 교차 검증 및 후처리}
\begin{substeps}
  \item 특정 시점 편향·과적합을 줄이고 시간적 강건성을 확보하기 위해 7일 슬라이딩 24-Fold Rolling CV + 앙상블로 일반화.
  \item 판매 수량이 정수라는 제약을 반영 --- 0과 1 사이 예측은 올림해 1로, 나머지는 반올림 후 정수 변환. 실적이 있는 메뉴를 0으로 예측하는 손실을 차단해 \textbf{약 2\% 향상}.
\end{substeps}

\achievement{오프라인 본선 진출(800여 팀, 1,600명), \textbf{최종 5위} --- 리더보드 Private 0.54969 / Public 0.55653. \textit{본선은 4인 팀 공동 작업.}}
