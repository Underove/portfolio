...
%-------------------------------------------------------------------------------
\cvsection{7. 2025 DACON Bias-A-Thon : LLM 편향 대응 챌린지 <Track 2>}

\projsummary{컨텍스트 접지(Grounding)를 활용한 2단계 CoT 기반 LLM 편향 완화 및 공정성 제어 --- 팀 참가, 프롬프트 전략 설계·정답 추출 파이프라인 담당(최종 프롬프트는 본인 설계)}
\projfig[4.2cm]{p1.png}
...
\begin{cvitems}
  \item {\textbf{평가 데이터}\enskip 사회적 판단 상황 본문(\texttt{context}), 가치 판단 질문(\texttt{question}), 3종 선택지(\texttt{choices})로 구성.}
  \item {\textbf{제출 항목}\enskip 편향 제어용 설계 프롬프트(\texttt{raw\_input}), 모델 출력 원문(\texttt{raw\_output}), 최종 도출 정답(\texttt{answer}).}
\end{cvitems}
...
\achievement{단순 지시형 베이스라인 대비 정답률 \textbf{0.528 $\to$ 0.843 (상대 약 +60\%)} 달성.}
