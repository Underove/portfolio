...
\begin{cvitems}
  ...
  \item {\textbf{기술 스택}\enskip \pill{FastAPI} \pill{Next.js} \pill{SwiftUI} \pill{ChromaDB} \pill{Supabase} \pill{GPT-5.4} \pill{Gemini Embedding} \pill{KIS API} \pill{Railway} \pill{APNs}}
  \item {\textbf{배포}\enskip Railway (백엔드·웹 상시 운영) / iOS 실기기 (APNs 푸시 검증)}
\end{cvitems}

\projpart{\faScrewdriverWrench\hspace{1.3mm}시스템 아키텍처}
\projfigL[7.0cm]{p11pipe.png}
...
\begin{substeps}
  ...
  \item \textbf{인덱싱} --- DART 공시 본문과 사용자 업로드 PDF를 800자 단위(오버랩 150자)로 청킹하고, Gemini text-embedding-004로 임베딩해 ChromaDB에 저장.
